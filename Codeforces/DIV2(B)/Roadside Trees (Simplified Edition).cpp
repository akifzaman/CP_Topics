...
void solve(){
    ll n, prevHeight=0, currentHeight=0, sum=0, eat=1, jump=1;
    cin>>n;
    ll arr[n]={0};
    for(ll i=0;i<n;i++){
        cin>>arr[i];
    ...
    for(ll i=0;i<n;i++){
        currentHeight=arr[i];
        if(i==0 && currentHeight > prevHeight){
            sum += (currentHeight - prevHeight);
        }
        if(i!=0 && currentHeight > prevHeight){
            sum += (currentHeight - prevHeight +jump);
        }
        if(currentHeight < prevHeight){
            sum += (prevHeight - currentHeight + jump);
        }
        if(currentHeight == prevHeight){
            sum += jump;
        }
